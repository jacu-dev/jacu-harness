...
// Living teaching surfaces must not tell anyone to type jacu-mcp. The retired
// string may remain only as the current module path, the current user-dir,
// the current GitHub repo, the JACU_MCP_* env prefix, the documented compat
// alias, or named historical/export contracts.
func TestLivingDocsDoNotTeachRetiredCommand(t *testing.T) {
	root := filepath.Join("..", "..")
	allowed := regexp.MustCompile(
		`github\.com/jacu-dev/jacu-mcp|jacu-dev/jacu-mcp|~?/?\.jacu-harness|JACU_MCP_[A-Z0-9_]+|"o jacu-mcp não chama modelo"|symlink ` + "`jacu-mcp`" + `|strings ` + "`jacu-mcp`" + `|private ` + "`jacu-mcp`",
	)
	retired := regexp.MustCompile(`jacu-mcp`)
	// Frozen ADRs record the old product name. SDD-016 and the one-shot plan
	// are the export contract and must still name the retired string.
	skipRel := map[string]bool{
		...
		"docs/plans/one-shot-open-source.md":              true,
	}
