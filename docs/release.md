...
5. Confirm `.github/workflows/release.yml` published a **non-draft**
   ...
   synced `Formula/jacu.rb` (workflow_dispatch **Sync formula**, or wait
   for the scheduled run). Refresh this repo's `Formula/jacu.rb` to the
   same file so the in-tree fallback stays honest.
